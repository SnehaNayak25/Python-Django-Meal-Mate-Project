    ...
    if request.method =="POST":
        ...
        try:
            ...
            if username == 'admin':
                ...
                return render(request,'delivery/cus_display_res.html',{'username':username, 'li':li})
        except:
...
def add_res(request):
    ...
    if form.is_valid():
        res_name = form.cleaned_data.get('Res_name')
        try:
...
def cus_display_res(request,username):
    customer = Customer.objects.get(username = username)
    li = Restaurants.objects.all()
    return render(request, 'delivery/cus_display_res.html',{'li':li,'username':username})

def cus_menu(request,id,username):
    res = Restaurants.objects.get(pk = id)
    menu = Menu.objects.filter(res = res)
    return render(request,'delivery/cus_menu.html',{'res':res, 'menu':menu, 'username':username})
...
def add_to_cart(request,menuid,username):
    ...
    return redirect('delivery:cus_menu',id = item.res.id, username = username)
